...
class CompletedRunQueries:
    ...
    def inspect(self, run_id: str, operation: Operation, *, owner: str,
                supplier_id: str | None = None, supplier_ids: list[str] | None = None) -> dict:
        ...
        def candidate(identifier: str | None) -> dict:
            matches = [c for c in candidates + withheld if identifier and c.get("supplier_id") == identifier]
            if len(matches) != 1:
                raise ValueError("Select a unique supplier_id from list_candidates for this run.")
            return matches[0]
        ...
        if operation == "get_candidate_evidence":
            selected = candidate(supplier_id)
            ids = {ref["ledger_id"] for ref in selected.get("evidence_refs", [])}
            return {"run_id": run_id, "supplier_id": supplier_id,
        ...
        if operation == "compare_candidates":
            identifiers = supplier_ids or []
            if not 2 <= len(identifiers) <= 10 or len(set(identifiers)) != len(identifiers):
                raise ValueError("Select 2 to 10 distinct supplier_ids from this run.")
            return {"run_id": run_id, "candidates": deepcopy([candidate(i) for i in identifiers])}
        if operation == "get_rfq_draft":
            selected = candidate(supplier_id)
            drafts = result.get("rfq_drafts", [])
            matches = [d for d in drafts if d.get("vendor", {}).get("supplier_id") == supplier_id]
            ...
            return {"run_id": run_id, "supplier_id": supplier_id, "submission_status": "unsent",
