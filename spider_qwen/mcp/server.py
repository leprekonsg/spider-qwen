...
def build_server():
    ...
    @server.tool()
    def get_current_run(run_id: str) -> dict:
        """Read metadata for a completed procurement run."""
        return handlers.inspect_run(run_id, "get_current_run")

    @server.tool()
    def list_candidates(run_id: str) -> dict:
        """List recorded suppliers with stable IDs, scores and evidence."""
