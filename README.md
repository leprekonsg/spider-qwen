...
Calibrating the statistical emission gate (hand-grading required). The gate is
a Learn-then-Test selective-risk threshold: calibrated, it bounds
P(wrong | emitted) <= alpha at confidence 1-delta; the split-conformal coverage
abstainer is reported alongside as an advisory recall diagnostic only:
...
- The selective-risk emission gate requires a hand-graded calibration set with
  BOTH correct and incorrect labels; it refuses to claim a guarantee when
  uncalibrated (zero-error certification needs >= 33 emitted examples at
  alpha=delta=0.1). `spider-qwen calibrate template|check` builds and validates
  the set; the grading itself stays human and must not come from the verifier
  being gated.
